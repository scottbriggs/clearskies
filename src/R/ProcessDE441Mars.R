# Extract data for Mars from the JPL ASCII development ephemeris (DE)
# files and store in parquet file format.

ProcessDE441Mars <- function(filename)
{
  ...
  mars_col_names <- c("Julian_Day_Start", "Julian_Day_End", "INTERVAL",
                      ...
                      "Z9", "Z10", "Z11")
  mars_data = matrix(0.0,nrow=11414,ncol=DE441NUMCOEFFMARS*3+3)
  colnames(mars_data) <- mars_col_names
  
  # Populate the intervals for Mars
  for (i in seq(from = 1L, to = 11414, by = 1L)){
